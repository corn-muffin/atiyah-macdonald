...
\begin{proof}
  (a$\implies$b) We first prove (L1).
  Let $\a\lideal A$ and take $\p\in \Spec A$.
  If $\a\not\subset \p$, then we can take $x\in \a\setminus \p$ so that $S_\p(\a)=(1)=(\a:x)$.

  Now, suppose we have $\a\subset \p$, and consider the isolated set of prime ideals 
  \[
    \Sigma=V(\p)\subset V(\a)
  \]
  over $\a$. 
